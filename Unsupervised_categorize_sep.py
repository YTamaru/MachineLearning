...
class Image_Clustering:
	def __init__(self, n_clusters=50, video_file='IMG_2140.MOV', image_file_temp='img_%s.png', input_video=False, col=7*7, channels=512, k_NN=4, hist=True):
	...
	def Bag_of_VisualWords(X, centr):
	...
	def BoVW_hist(X_vector):
		...
		# 		L2 = sorted(range(len(L2)), key=lambda k: L2[k])[:self.k_NN] #リストの値を昇順に並び替えて元の要素のインデックスの値で返す 上位k個→ダメ
		# 		if L2[0] != node_I[0]:
		# 			node_I.append(L2[0])
		# 		else:
		# 			break
			...
			for i in range(len(X_list)-1):
				...
				if L2[0,1] != node_I[0]:
					...
					# for m in range(len(L2)-1):
						# DG.add_edges_from([(L2[0,0],L2[m,1])])
				else:
					break
			cycle.append(node_I)
			for l in range(len(node_I)):
