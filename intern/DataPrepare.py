...
class DataPrepare:
    ...
    def __init__(self, dataFrame, labels=None):
        ...
        self.newest_data = self.create_newest_data(dataFrame)
    ...
    def create_newest_data(self,data_sets):
        data_sets = data_sets.ix[- self.time_step : ,:]
        scaled_data_sets = {}        
        for label in self.labels:
            scaled_data_sets[label] = self.scaler[label].transform(data_sets.ix[:, label])
        x = []
        for j in range(self.time_step):
            time_slice = []
            for key in self.labels:
                time_slice.append(scaled_data_sets[key][j])
            x.append(time_slice)
        x =  numpy.array(x)
        x = numpy.reshape(x, (1,x.shape[0], x.shape[1]))
        return x
